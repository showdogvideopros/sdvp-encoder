...
// ⛔ AN encode_s FROM A CONCURRENT RUN AND ONE FROM A SEQUENTIAL RUN ARE NOT
// THE SAME QUANTITY. [MEASURED 2026-09-01] the same 91-min film took 6132.7 s
// encoding one codec at a time and 4044.9 s with both overlapped and pinned.
// Pooled into one fit those rows describe no machine that exists.
//
// ⛔ AND THE DIFFERENCE IS NOT ONE NUMBER. Concurrency bought 1.49x on the
// two-socket box and 1.07x on the single-socket one - so a global correction
// factor would be wrong on both. The MODE STRING already names the arrangement
// that produced each row; the fit simply asks for its own.
//
// EARN IT OR FALL BACK, the same rule the HEVC coefficient follows: a mode-
// specific fit is used only once it has MIN_FIT_FILMS behind it. Below that
// the POOLED fit is used unchanged - so this changes nothing until the record
// can support it, and corrects itself without anyone deciding a date.
//
// mode === null asks for the pooled fit explicitly.
function fitEncode(db, mode) {
  let rows;
  const SQL = "SELECT m.duration_s dur, m.encode_s enc, " +
      "GROUP_CONCAT(r.codec || ':' || r.height) hs " +
      "FROM movies m JOIN rungs r ON r.item_id = m.item_id " +
      "WHERE m.encode_s > 0 AND m.duration_s > 0 " +
      "AND r.state IN ('STORED','VERIFIED') ";
  try {
    if (mode) {
      rows = db.prepare(SQL + "AND m.encode_mode = ? GROUP BY m.item_id").all(mode);
      // Not enough of its own kind yet - say so by returning null, and let the
      // caller fall back rather than silently fitting the wrong population.
      if (!rows || rows.length < MIN_FIT_FILMS) return null;
    } else {
      rows = db.prepare(SQL + "GROUP BY m.item_id").all();
    }
  } catch (e) { return null; }
  ...
  return { base: Math.max(0, c[0]), h264: c[1], hevc: hevc,
           ...
           hevc_measured: hevcOk, mode: mode || null };
}

// mode: the encode_mode the NEXT run is expected to use. Optional - called
// bare, everything behaves exactly as it did before this argument existed.
function rates(mode) {
  let db = null;
  try {
    ...
    // Ask for this mode's own fit; fall back to the pooled one when the
    // record cannot yet support it. out.fit.mode names which was used, so a
    // caller can tell the reader whether the figure is mode-specific.
    out.fit = (mode ? fitEncode(db, mode) : null) || fitEncode(db, null);
    out.fit_mode_requested = mode || null;
    db.close();
    return out;
  } catch (e) {
