...
function runProbe(file, durationS, outDir, srcName, codec, opts) {
  ...
  // 'rate' lived here to feed the fps filter, which is gone - the sheet now
  // seeks to each tile rather than decoding through the film to find it.
  ...
  return (async () => {
    ...
    // ⭐ SEEK TO THE TILES, NEVER DECODE THE WHOLE FILM.
    // The old form asked ffmpeg for 40 frames spread across the movie using an
    // fps filter - which reaches them by DECODING EVERY FRAME and discarding
    // 99.9% of them. MEASURED 2026-09-01 on a 23.7-min pair, same 8x5 sheet:
    //     h264   32.0 s -> 8.1 s   (4x)
    //     hevc   93.3 s -> 11.7 s  (8x)
    // Sheets compared side by side by Dr. K on both codecs: indistinguishable,
    // a clean spread of 40 different frames. The HEVC saving is the one that
    // matters - that rung dominated a 383 s probe phase on a 91-min film.
    //
    // -ss BEFORE -i is a fast seek to the nearest keyframe, so a tile may sit a
    // fraction of a second from its nominal timestamp. Irrelevant to what the
    // sheet is FOR: blue frames, dropouts, stuck overlays, gross blocking.
    //
    // ⛔ ONE PROCESS WITH 40 SEEKED INPUTS WAS TRIED AND IS WRONG. It ran in
    // 2.15 s and produced FORTY COPIES OF ONE FRAME - fast because it did a
    // fortieth of the work. Caught by eye, not by any check in the code. Do not
    // reach for it again without comparing the image.
    const nTiles = SHEET_TILES_X * SHEET_TILES_Y;
    const tileDir = path.join(outDir, '.tiles-' + String(codec));
    let s = { code: 0, err: '', wall: 0 };
    const tileT0 = Date.now();
    try {
      fs.rmSync(tileDir, { recursive: true, force: true });
      fs.mkdirSync(tileDir, { recursive: true });
      const dur = (isFinite(durationS) && durationS > 0) ? Number(durationS) : 0;
      let made = 0, firstErr = '';
      for (let i = 0; i < nTiles; i++) {
        // Midpoint of each slice, so neither the first nor the last tile lands
        // on a black frame at the very edge of the film.
        const ts = dur > 0 ? (((i + 0.5) * dur) / nTiles) : 0;
        const tile = path.join(tileDir, String(i + 1).padStart(3, '0') + '.jpg');
        const r = await pass([
          '-hide_banner', '-v', 'error', '-nostdin', '-nostats',
          '-ss', ts.toFixed(3),
          '-i', file,
          '-frames:v', '1',
          '-vf', 'scale=' + SHEET_TILE_W + ':-2',
          '-q:v', String(SHEET_QUALITY), '-y', tile
        ]);
        if (r.code === 0 && fs.existsSync(tile) && fs.statSync(tile).size > 0) made++;
        else if (!firstErr) firstErr = 'tile ' + (i + 1) + ' at ' + ts.toFixed(3) + 's: ' +
                                       (r.err || 'exit ' + r.code).trim().split('\n').slice(-1)[0];
      }
      // ⛔ A SHORT SHEET IS A FAILURE, NOT A SMALLER SHEET. 39 tiles in an 8x5
      // grid renders and looks almost right, and nothing downstream would ever
      // question it. Refuse instead.
      if (made !== nTiles) {
        s = { code: -1, wall: (Date.now() - tileT0) / 1000,
              err: 'contact sheet: only ' + made + ' of ' + nTiles + ' tiles' +
                   (firstErr ? ' - ' + firstErr : '') };
      } else {
        const a = await pass([
          '-hide_banner', '-v', 'error', '-nostdin', '-nostats',
          '-pattern_type', 'glob', '-i', path.join(tileDir, '*.jpg'),
          '-vf', 'tile=' + SHEET_TILES_X + 'x' + SHEET_TILES_Y,
          '-frames:v', '1', '-q:v', String(SHEET_QUALITY), '-y', sheet
        ]);
        s = { code: a.code, err: a.err, wall: (Date.now() - tileT0) / 1000 };
      }
    } catch (e) {
      s = { code: -1, err: 'contact sheet: ' + e.message,
            wall: (Date.now() - tileT0) / 1000 };
    } finally {
      // The tiles live INSIDE outDir, which is uploaded by explicit filename
      // (quality.sheets and audio_report), never by directory sweep - so they
      // could not ship. Removed anyway: scratch survives a FAILED film.
      try { fs.rmSync(tileDir, { recursive: true, force: true }); } catch (e2) {}
    }
