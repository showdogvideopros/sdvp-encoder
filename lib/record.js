...
-- ARTIFACTS. Everything we send to a destination that is NOT a video rung:
-- contact sheets, the plain-text audio report, the manifest. Before this table
-- the record held only quality.sheets_uploaded - a count the daemon derived
-- from its OWN intentions, with no vendor answer behind it. [MEASURED
-- 2026-08-21] pCloud already returns a fileid for all three kinds and a
-- verified sha1 for sheets; every one of them was discarded a line later.
-- Same evidence class as a rung delivery, or the report cannot speak to them.
CREATE TABLE IF NOT EXISTS artifacts (
  ...
  error         TEXT,
  recorded_at   TEXT,
  UNIQUE (item_id, kind, codec)
);
`;

let _db = null;

// COLUMN MIGRATION. CREATE TABLE IF NOT EXISTS is silent about columns it does
// not add - it sees the table exists and stops. [MEASURED 2026-08-21] no ALTER
// path existed anywhere in this module and schema_version had never been read
// as a decision, so a column named in an INSERT would have thrown on the first
// real film of a real run.
//
// THIS LIST IS THE ONE PLACE. A new column is added here, not in SCHEMA as
// well - two places to edit is one place to forget.
const COLUMNS = [
  // The scratch directory a FAILED film leaves behind, retained deliberately
  // for inspection. The path was known and thrown away; nothing could tell
  // the operator where the disk was going.
  ['movies', 'scratch_path', 'TEXT'],

  // SILENCE. [MEASURED 2026-08-21] flat factor was retired as a gate - the
  // comparison was inverted AND compression erases flatness on every master we
  // handle. Silence detection replaced it: a fade, a slo-mo passage with no
  // audio, and a pulled microphone are all distinguishable by HOW LONG and
  // WHERE. Two real masters each carried one 1.819271 s stretch at the very
  // end - the edit template, not content.
  // ⛔ RECORDED ONLY. No threshold until a corpus exists.
  //
  // These four also REPLACE flat_max as the old-probe discriminator (Dr. K's
  // ruling 2026-08-21): a perfectly flat channel now reads -Infinity, which
  // SQLite cannot store and which lands as NULL in flat_max - colliding with
  // "never measured". No row written before today can carry silence_n.
  ['quality', 'silence_n', 'INTEGER'],
  ['quality', 'silence_total_s', 'REAL'],
  ['quality', 'silence_pct', 'REAL'],
  ['quality', 'silence_longest_s', 'REAL'],
  ['quality', 'silence_longest_at_s', 'REAL'],
  ['quality', 'silence_ends_at_end', 'INTEGER'],
  ['rungs', 'rate_kbps', 'INTEGER'],
  // ENCODE MODE. Post-concurrency, encode_wall_s measures a CONTENDED encode.
  // Mixed with sequential rows the fitted model degrades and nothing flags it.
  // 'seq' | 'conc:12/16' | 'conc:16/16' | 'conc:none'. NULL = written before
  // this column existed; NOT backfilled, because that value was never measured.
  // SCHEMA_VERSION deliberately NOT bumped: ensureColumns runs every startup
  // and is not gated on it, and bumping would rewrite migrated_at for nothing.
  ['rungs', 'encode_mode', 'TEXT'],
  ['movies', 'encode_mode', 'TEXT']
];
...
function ensureColumns(d) {
  ...
  for (const c of COLUMNS) {
    const have = d.prepare(
      'SELECT COUNT(*) AS c FROM pragma_table_info(?) WHERE name = ?').get(c[0], c[1]).c;
    if (!have) { d.exec('ALTER TABLE ' + c[0] + ' ADD COLUMN ' + c[1] + ' ' + c[2]);
                 added.push(c[0] + '.' + c[1]); }
  }
  return added;
}
...
function db() {
  ...
  _db.exec(SCHEMA);
  const added = ensureColumns(_db);
  if (added.length) console.log('record: added column(s) ' + added.join(', '));
...
function upsertMovie(runId, item) {
  ...
  db().prepare(`INSERT INTO movies
    (item_id, run_id, source_name, source_fileid, source_bytes, show_path, dest_path,
     ...
     source_mbps, phase, error, fetch_s, encode_s, verify_s, upload_s, encode_mode)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    ON CONFLICT(item_id) DO UPDATE SET
      ...
      encode_mode = excluded.encode_mode`)
...
function upsertRung(itemId, out, r, spec, capMbps) {
  const stmt = db().prepare(`INSERT INTO rungs
    (item_id, codec, preset_name, encoder, encoder_preset, height, state, reason,
     ...
     pct_of_cap, verify_s, encode_wall_s, speed_x, rate_kbps, encode_mode)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    ON CONFLICT(item_id, codec, height) DO UPDATE SET
      ...
      speed_x = excluded.speed_x, encode_mode = excluded.encode_mode
    RETURNING rung_id`);
  ...
  const row = stmt.get(itemId, out.codec || null, out.preset_name || null,
    ...
    // WHAT WAS ACTUALLY TARGETED. The rung's own rate wins (the derived top
    // rung sets it per film); the preset's figure is the fallback. Without
    // this the record shows a ceiling that never engaged and no target at all.
    int(r.rate_kbps != null ? r.rate_kbps : (spec && spec.rate_kbps)),
    (out && out.encode_mode) || null);
...
function upsertQuality(itemId, q) {
  ...
  db().prepare(`INSERT INTO quality
    (item_id, audio_ok, audio_from_height, channels, peak_max_db, rms_min_db,
     ...
     sheets_uploaded, probe_s, silence_n, silence_total_s, silence_pct,
     silence_longest_s, silence_longest_at_s, silence_ends_at_end)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    ON CONFLICT(item_id) DO UPDATE SET
      ...
      silence_n = excluded.silence_n, silence_total_s = excluded.silence_total_s,
      silence_pct = excluded.silence_pct, silence_longest_s = excluded.silence_longest_s,
      silence_longest_at_s = excluded.silence_longest_at_s,
      silence_ends_at_end = excluded.silence_ends_at_end`)
   .run(itemId, bool(a && a.ok), int(a && a.from_height), int(rec && rec.channels),
        ...
        num(rec && rec.imbalance_db), num(rec && rec.flat_max),
        ...
        int(sil && sil.n), num(sil && sil.total_s), num(sil && sil.pct),
        num(sil && sil.longest_s), num(sil && sil.longest_at_s),
        sil ? bool(sil.ends_at_end) : null);
}

// One row per artifact sent anywhere. Keyed (item, kind, codec) so a re-record
// updates rather than duplicates; codec is '' rather than NULL because SQLite
// treats NULLs as distinct in a UNIQUE index and manifests would multiply.
function upsertArtifacts(itemId, arts) {
  if (!arts || !arts.length) return;
  const stmt = db().prepare(`INSERT INTO artifacts
    (item_id, kind, codec, name, bytes, destination, dest_path, ok,
     pcloud_fileid, sha1_verified, error, recorded_at)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
    ON CONFLICT(item_id, kind, codec) DO UPDATE SET
      name = excluded.name, bytes = excluded.bytes,
      destination = excluded.destination, dest_path = excluded.dest_path,
      ok = excluded.ok, pcloud_fileid = excluded.pcloud_fileid,
      sha1_verified = excluded.sha1_verified, error = excluded.error,
      recorded_at = excluded.recorded_at`);
  const now = new Date().toISOString();
  for (const a of arts) {
    if (!a || !a.kind) continue;
    stmt.run(itemId, String(a.kind), a.codec == null ? '' : String(a.codec),
             a.name || null, int(a.bytes), a.destination || 'pcloud',
             a.dest_path || null, bool(a.ok), int(a.pcloud_fileid),
             a.sha1_verified == null ? null : bool(a.sha1_verified),
             a.error || null, now);
  }
}
...
// WHAT WE ALREADY KNOW ABOUT A MASTER, without fetching it. The cheap
// existence check must rebuild a film's plan BEFORE the download, and the
// planner needs only the master's height and codec to decide which rungs to
// drop. Both are recorded the first time a film is seen.
// ⛔ The manifest reaches into the probe for five fields. A film skipped
// without a fetch would write five nulls - the exact failure run 7 paid for.
// This carries the source block forward instead, the same way priorRung
// carries a rung forward.
// Only a film that reached at least PROBING has usable dimensions, so rows
// that failed earlier are excluded rather than returned half-empty.
function priorSource(sourceName) {
  const row = db().prepare(`
    SELECT source_name, source_fileid, source_bytes, width, height, fps,
           duration_s, video_codec, audio_codec, has_audio, source_mbps
    FROM movies
    WHERE source_name = ? AND width IS NOT NULL AND height IS NOT NULL
      AND duration_s IS NOT NULL
    ORDER BY rowid DESC LIMIT 1`).get(sourceName);
  return row || null;
}
...
module.exports = { db, close, upsertArtifacts, stats, query, findDelivery, priorRung, priorSource, DB_PATH, SCHEMA_VERSION,
