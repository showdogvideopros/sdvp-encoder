...
function runEncode(opts) {
  ...
  return new Promise((resolve, reject) => {
    const t0 = Date.now();
    // CPU AFFINITY. overrides.cpu_mask is a taskset list ('0-11'). When absent
    // the spawn is EXACTLY what it has always been - no taskset in the picture.
    // ⛔ MEASURED 2026-08-31: pinning a LONE codec to 16 threads took 790.9 s
    // against 505.4 s unpinned - 56% SLOWER. The caller decides; this function
    // only obeys. Never pass a mask for a single-encoder film.
    const mask = (overrides && overrides.cpu_mask) || null;
    const cmd  = mask ? TASKSET : FFMPEG;
    const argv = mask ? ['-c', String(mask), FFMPEG].concat(built.args) : built.args;
