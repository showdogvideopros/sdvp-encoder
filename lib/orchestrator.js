...
function buildRun(job, jobPath) {
  ...
  // PRICE THE RUN ONCE, HERE, BEFORE ANYTHING STARTS. The run card projected
  // from elapsed/done only, so with one film finished the whole forecast rested
  // on one film and swung on the next completion. This is the PRIOR it lacked:
  // the fitted picture-area model, which prices from rungs and bytes and needs
  // nothing to have happened yet. Masters run 10.32-10.43 Mbps on this material,
  // so bytes give duration and the shape of a run is knowable at t=0.
  // Uses outputs_requested (the PLAN), never outputs (populated per film later).
  // SWALLOWED DELIBERATELY, as recordArtifacts is: a cosmetic figure must never
  // be able to stop a run from starting.
  try {
    const EST = require('./estimate.js');
    const films = run.items.map(function (it) {
      const rungs = [];
      for (const o of it.outputs_requested || [])
        for (const h of o.rungs || [])
          rungs.push({ codec: o.codec, height: Number(h) });
      return { bytes: it.bytes || 0, rungs: rungs };
    }).filter(function (f) { return f.bytes > 0 && f.rungs.length; });
    // ⛔ PRICE THE RUN IN THE MODE IT WILL ACTUALLY RUN IN. Derived here the
    // same way the encode loop derives it, from the same cpuPlan(), so the two
    // agree by construction rather than by coincidence.
    //
    // ⚠️ IT IS A PREDICTION, NOT A READING. The loop counts outputs whose rungs
    // are still PLANNED, which depends on what already exists at the
    // destination - unknown at pricing time. A film whose HEVC is already
    // delivered will run 'seq' and be priced as concurrent. Acceptable for an
    // estimate; stated here so nobody later reads it as a measurement.
    let priceMode = 'seq';
    try {
      const maxCodecs = films.reduce(function (n, f) {
        const set = {};
        for (const r of f.rungs || []) set[r.codec] = 1;
        return Math.max(n, Object.keys(set).length);
      }, 0);
      if (maxCodecs > 1) {
        const plan = cpuPlan();
        priceMode = plan ? ('conc:' + plan.label) : 'conc:none';
      }
    } catch (e) { priceMode = 'seq'; }
    if (films.length === run.items.length && films.length) {
      const e = EST.estimate(films, EST.rates(priceMode));
      run.priced_s = e.seconds;
      run.priced_low = e.low;
      run.priced_high = e.high;
    }
  } catch (e) {
    console.error('run pricing failed (run continues): ' + String(e.message).slice(0, 200));
  }
  return run;
}
...
// ---- CPU PLAN. Reads the kernel's own socket map and returns taskset lists,
// or NULL when the hardware is not the shape the measurement was taken on.
//
// ⛔ NULL IS THE SAFE ANSWER AND THE DEFAULT. A wrong mask is worse than none:
// MEASURED 2026-08-31, a lone H.264 pinned to 16 threads ran 790.9 s against
// 505.4 s unpinned. This function never guesses.
//
// The split is a FRACTION, not the literal numbers 12 and 16. On the box it was
// measured on that yields 12/16; on other hardware it yields the same shape.
function cpuPlan() {
  try {
    const base = '/sys/devices/system/cpu';
    const socketOf = {};
    for (const name of fs.readdirSync(base)) {
      const m = /^cpu(\d+)$/.exec(name);
      if (!m) continue;
      const f = path.join(base, name, 'topology', 'physical_package_id');
      if (!fs.existsSync(f)) return null;
      const sid = String(fs.readFileSync(f, 'utf8')).trim();
      (socketOf[sid] = socketOf[sid] || []).push(Number(m[1]));
    }
    const ids = Object.keys(socketOf).sort();
    if (ids.length !== 2) return null;                       // 1 socket, or 3+
    const a = socketOf[ids[0]].sort((x, y) => x - y);
    const b = socketOf[ids[1]].sort((x, y) => x - y);
    if (a.length !== b.length) return null;                  // asymmetric
    if (a.length < 16) return null;                          // too small to split
    // H.264 gives back the top quarter of its socket. Those threads stay IDLE
    // on purpose - the machine is memory-bound and ran FASTER with them free.
    const keep = a.length - Math.floor(a.length / 4);
    const h264 = a.slice(0, keep);
    const hevc = b;
    const range = (list) => list[0] + '-' + list[list.length - 1];
    return { h264: range(h264), hevc: range(hevc),
             label: h264.length + '/' + hevc.length,
             sockets: ids.length, per_socket: a.length };
  } catch (e) {
    return null;
  }
}
  ...
  try {
    ...
    for (const o of item.outputs_requested) {
      ...
      for (const r of plan.rungs) {
        ...
        // ⭐ BOTH 1080p RUNGS DERIVE. Ruled 2026-08-25. HEVC ran at a FIXED
        // 3500 and crossed 4 GiB past ~152 min, so the H.264 rung was being
        // carefully refused for exceeding a ceiling the HEVC rung then
        // exceeded silently. Same arithmetic, per-codec floor.
        // With both deriving, EVERY file this box makes is under 4 GiB.
        const FLOORS = { h264: 3500, hevc: 2500 };
        if (Number(r.height) === 1080 && FLOORS[plan.codec]) {
          let dps = null;
          ...
          if (spec && spec.rate_kbps && dur > 0) {
            ...
            const FLOOR_KBPS = FLOORS[plan.codec];
            ...
            if (chosen < FLOOR_KBPS) {
              r.state = 'SKIPPED';
              r.reason = 'film too long for a 1080p ' +
                (plan.codec === 'hevc' ? 'HEVC' : 'H.264') + ' file under 4 GB';
              continue;
            }
            r.rate_kbps = chosen;
    ...
    // ENCODE MODE - what this loop ACTUALLY DID, recorded beside the timings.
    // Today the outputs run one after another, so every row is 'seq'. When
    // concurrency ships this must be derived from the branch TAKEN, not the
    // branch configured: a box that degrades to sequential must record 'seq'.
    // Assigned below from the number of encoders ACTUALLY LAUNCHED.
    let ENCODE_MODE = 'seq';
    // FILM PROGRESS ACROSS PASSES. Each output is its own ffmpeg run, so a
    // single film-level bar fed by the live pass FILLS, RESETS TO ZERO and
    // FILLS AGAIN. Weights MEASURED 2026-08-24 (run 45, 10 films, 16-133 min):
    // the H.264 pass is 44% of a film's encode time, HEVC 56%.
    // ⛔ THE SAME WEIGHTS LIVE IN public/status.html's run projection.
    //    If one changes the other MUST. They are not derived from each other.
    const PASS_W = { h264: 0.44, hevc: 0.56 };
    const filmPct = () => {
      const outs = item.outputs || [];
      let tot = 0, got = 0;
      for (const o of outs) {
        const w = (PASS_W[o.codec] != null) ? PASS_W[o.codec] : (1 / outs.length);
        tot += w;
        const rs = o.rungs || [];
        const anyPlanned = rs.some(x => x.state === 'PLANNED');
        if (o.encoding && o.progress) {
          got += w * (Math.max(0, Math.min(100, o.progress.pct || 0)) / 100);
        } else if (rs.length && !anyPlanned) {
          got += w;   // finished, or skipped as already at destination
        }
      }
    ...
    mark('ENCODING');
    t = Date.now();
    // ---- LAUNCH EVERY PLANNED OUTPUT AT ONCE.
    // MEASURED 2026-08-31 on a 32-core box: the two codecs overlapped run the
    // full ladder in 1106-1217 s against 1482 s sequential. The machine is
    // MEMORY-BANDWIDTH bound, not core bound - which is why the gain is real
    // but smaller than the core count suggests.
    // ⛔ FAILURE POLICY B (Dr. K, 2026-09-01): if one codec fails while the
    // other is still working, THE SURVIVOR IS NOT CANCELLED. It runs to
    // completion, its outputs keep their real state, and a re-run skips what
    // already exists. The film still FAILS - allSettled then a rethrow - so
    // nothing ships silently half-encoded.
    const launched = [];
    for (const out of item.outputs) {
      const todo = out.rungs.filter(r => r.state === 'PLANNED');
      if (!todo.length) continue;
      const preset = loadPreset(out.preset_name);
      out.progress = { pct: 0, speed_x: 0, eta_s: null };
      out.encoding = true;
      launched.push({ out: out, todo: todo, preset: preset });
    }
    // ⛔ THE MODE IS WHAT HAPPENED, NOT WHAT WAS CONFIGURED. A film whose HEVC
    // is already at the destination launches ONE encoder and is sequential,
    // however the box is set up. The estimator's fitted model depends on this
    // distinction and nothing else would flag it.
    // ---- CPU AFFINITY, derived from the kernel's own socket map.
    // MEASURED 2026-08-31 on a 2-socket 32-thread box, same film both ways:
    //   unpinned overlap 1216.9 s · 16/16 pinned 1135.2 s · 12/16 pinned 1106.1 s
    // Four IDLE threads made the machine FASTER. The box is memory-bandwidth
    // bound, not core bound, so the winning split leaves headroom on purpose.
    // ⛔ ONE SOCKET = NO MASKS. enc is a single-socket 16-thread box where the
    // split has no meaning and pinning is a large loss. Anything the rule does
    // not recognise degrades to no pinning, never to a guess.
    const plan = (launched.length > 1) ? cpuPlan() : null;
    if (plan) {
      const byCodec = { h264: plan.h264, hevc: plan.hevc };
      for (const L of launched) {
        const m = byCodec[L.out.codec];
        if (m) L.cpu_mask = m;
      }
      // If any launched output has no mask of its own, pin NOTHING - a mix of
      // pinned and unpinned encoders is a configuration nobody measured.
      if (launched.some(L => !L.cpu_mask)) for (const L of launched) L.cpu_mask = null;
    }
    const pinned = !!(plan && launched.every(L => L.cpu_mask));
    ENCODE_MODE = (launched.length > 1)
      ? (pinned ? ('conc:' + plan.label) : 'conc:none')
      : 'seq';
    for (const L of launched) L.out.encode_mode = ENCODE_MODE;
    for (const L of launched) L.out.cpu_mask = L.cpu_mask || null;
    save({ force: true });

    const settled = await Promise.allSettled(launched.map(function (L) {
      return E.runEncode({
        src, plannedRungs: L.todo, preset: L.preset, outDir, srcName: item.name,
        durationS: item.probe.duration_s,
        overrides: L.cpu_mask ? { cpu_mask: L.cpu_mask } : undefined,
        onProgress: (p) => {
          // PER-OUTPUT first, then the film's combined figure. Two encoders
          // reach this callback; each owns its own object and neither
          // overwrites the other's percentage. save() is throttled to one
          // write per 500 ms across ALL callers and writes atomically.
          L.out.progress = { pct: p.pct, speed_x: p.speed_x, eta_s: p.eta_s };
          item.progress.pct = filmPct();
          // FILM-LEVEL speed and eta describe THE OUTPUT THE FILM IS WAITING
          // ON - the one with the most TIME left - not whichever ticked last,
          // and NOT the lowest percentage. Percentage is not time: HEVC at 40%
          // may well finish before H.264 at 90%. runEncode reports no eta below
          // 0.5% of a pass, so fall back to lowest pct while eta is still null.
          let slow = null;
          for (const M of launched) {
            if (!M.out.encoding || !M.out.progress) continue;
            if (!slow) { slow = M.out; continue; }
            const a = M.out.progress.eta_s, b = slow.progress.eta_s;
            if (a != null && b != null) { if (a > b) slow = M.out; }
            else if (a != null && b == null) { slow = M.out; }
            else if (a == null && b == null) {
              if ((M.out.progress.pct || 0) < (slow.progress.pct || 0)) slow = M.out;
            }
          }
          item.progress.speed_x = slow ? slow.progress.speed_x : p.speed_x;
          item.progress.eta_s   = slow ? slow.progress.eta_s   : p.eta_s;
    ...
    for (let i = 0; i < settled.length; i++) {
      ...
      if (s.status === 'fulfilled') {
        ...
        if (out.progress) out.progress.pct = 100;
        out.encode_wall_s = res.wall_s;
        out.speed_x = res.speed_x;
      } else {
        const msg = pc.redact(String(s.reason && s.reason.message || s.reason)).split('\n')[0].slice(0, 200);
        for (const r of out.rungs) {
          if (r.state === 'PLANNED') { r.state = 'FAILED'; r.reason = 'encode: ' + msg; }
        }
        out.encode_error = msg;
        failedCodecs.push(out.codec + ': ' + msg);
      }
    }
    item.progress.pct = filmPct();
    save({ force: true });
    // ⛔ EVERY failed codec is named. With one encoder a single message was
    // the whole story; with two, reporting only the first loses the other.
    if (failedCodecs.length) throw new Error('encode failed - ' + failedCodecs.join(' | '));
    item.timings.encode_s = (Date.now() - t) / 1000;
    item.encode_mode = ENCODE_MODE;
    ...
    const sendToPCloud = async function (out, r) {
      ...
      // ⛔ KEEP THE EVIDENCE. [MEASURED 2026-09-01, run 54] one delivery in
      // 1,099 reported a mismatch on a file that was, and still is, CORRECT at
      // the destination - identical sha1, identical size, decodes clean. Four
      // hypotheses were tested and every one was ruled out: slow hash (correct
      // at 0 ms, four times), non-atomic overwrite (new hash returned
      // immediately, same fileid), a broken comparison (both sibling rungs
      // matched), corrupt bytes (they are not). THE EVENT IS UNEXPLAINED, and
      // it is unexplained because the ONE value that would have settled it -
      // what storage actually answered - was reduced to a boolean and thrown
      // away. No retry and no guard were added: this path has delivered 1,098
      // files correctly and a retry would hide a real corruption. Only the
      // instrument that did not exist.
      if (!r.checksum_match) {
        r.checksum_evidence = {
          remote_sha1: rk.sha1 || null,
          local_sha1: localSha,
          remote_size: (rk.size == null) ? null : rk.size,
          local_bytes: (r.bytes == null) ? null : r.bytes,
          fileid: up.fileid,
      ...
      if (!r.checksum_match) {
        // Prefixes, not full hashes: the reason travels into fields truncated
        // at 200-300 chars downstream, and 12 characters is ample to tell
        // "storage said nothing" from "storage said something else".
        const ev = r.checksum_evidence || {};
        const sh = v => (v == null || v === '') ? JSON.stringify(v) : String(v).slice(0, 12);
        r.reason = 'checksum mismatch after upload - storage said ' + sh(ev.remote_sha1) +
                   ' size ' + ev.remote_size + ', local ' + sh(localSha) +
                   ' size ' + ev.local_bytes;
      }
...
async function runJobFile(jobPath, state, save) {
  ...
  // ---- PAUSE: THE JOB GOES BACK TO STAGING
  // The film in flight has finished and been delivered. Rename the .accepted
  // job file into held/ so the SAME job can be released again, and force
  // on_existing to skip - a paused job is a continuation, whatever it was
  // built with, and re-encoding what is already delivered is never wanted.
  if (run.stopped_by_operator === 'PAUSE' && jobPath && jobPath.endsWith('.accepted')) {
    try {
      const j = JSON.parse(fs.readFileSync(jobPath, 'utf8'));
      j.on_existing = 'skip';
      j.paused_at = new Date().toISOString();
      const base = path.basename(jobPath).replace(/\.accepted$/, '');
      const dest = path.join(path.dirname(path.dirname(jobPath)), 'held', base);
      fs.writeFileSync(dest, JSON.stringify(j, null, 2));
      fs.unlinkSync(jobPath);
      run.paused_to = dest;
    } catch (e) {
      run.pause_error = String(e.message).slice(0, 200);
    }
  }
