  ...
  try {
    ...
    for (const out of item.outputs) {
      ...
      const res = await E.runEncode({
        ...
        onProgress: (p) => {
          // PER-OUTPUT first, then the film's combined figure. Under
          // concurrency two encoders reach this callback; each owns its
          // own object and neither overwrites the other's percentage.
          out.progress = { pct: p.pct, speed_x: p.speed_x, eta_s: p.eta_s };
