        ...
        for (const d of (r.destinations || [])) {
          if (d === 'pcloud' && r.stored_fileid) {
            ...
            R.upsertDelivery(rungId, 'vimeo', {
              ...
              witness_drift_s: null, upload_s: r.vimeo_s,
    ...
    if (probeTargets.length) {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        if (tgt.audio) {
          ...
          try {
            ...
            chans.forEach(function (c) {
              L.push('  channel ' + c.channel +
                     ...
                     '   flat factor ' + (c.flat_factor != null ? c.flat_factor.toFixed(2) : '-'));
            });
            const rec = j.recorded || {};
            L.push('');
            L.push('  channel imbalance : ' + (rec.imbalance_db != null ? rec.imbalance_db : '-') + ' dB');
            L.push('');
            L.push('WHAT IS GATED   : dead channel, silent channel, sustained flatness.');
            L.push('WHAT IS NOT     : peak and RMS levels, channel imbalance. These are');
            L.push('                  recorded only. No corpus exists yet to draw honest');
            L.push('                  thresholds from, so they do not fail anything.');
          ...
          // A dead or flat channel FAILS the rung it was measured on.
          if (!j.ok) {
            tgt.r.state = 'FAILED';
            tgt.r.reason = 'audio: ' + j.checks.filter(c => !c.ok)
                                        .map(c => c.name).join(',');
          }
        }
      }
...
async function runJobFile(jobPath, state, save) {
  ...
  run.status = !allDone ? 'COMPLETE_WITH_FAILURES'
