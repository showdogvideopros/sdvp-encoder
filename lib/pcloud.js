...
// ⛔ THE ONLY DELETE THIS CLIENT WILL EVER PERFORM, and it is deliberately
// narrow. Dr. K's standing rule: NOTHING IS EVER DELETED AT A DESTINATION.
// Until 2026-08-21 this client had no delete at all, which enforced that rule
// structurally rather than by memory.
//
// Record snapshots are the one exception he ruled on: one is written at the end
// of EVERY run, each is a complete copy of the whole record, and nothing pruned
// them. A general delete would have satisfied that need and handed every future
// caller the power to remove a customer's film. Dr. K chose the narrow form.
//
// So this refuses ANY name that is not a snapshot written by our own tool. A
// future caller wanting to prune something else at pCloud must add its own
// narrow function and make that decision deliberately, rather than inherit a
// capability nobody chose.
const SNAPSHOT_NAME = /^sdvp-encoder-record_\d{8}-\d{6}_.*\.db$/;

async function deleteSnapshot(fileid, expectedName) {
  const nm = String(expectedName || '');
  if (!SNAPSHOT_NAME.test(nm)) {
    throw new Error('refusing to delete "' + nm.slice(0, 60) + '" - not a record snapshot');
  }
  if (!fileid) throw new Error('refusing to delete: no file id');
  // READ THE OBJECT BEFORE ACTING ON IT. A fileid and a name arriving together
  // from a listing can still be mismatched by a caller; ask pCloud what that id
  // actually points at, and refuse if it is not the file we were told.
  const meta = await stat(fileid);
  if (String(meta.name) !== nm) {
    throw new Error('refusing to delete: id ' + fileid + ' is "' +
                    String(meta.name).slice(0, 60) + '", not "' + nm.slice(0, 60) + '"');
  }
  if (!SNAPSHOT_NAME.test(String(meta.name))) {
    throw new Error('refusing to delete: the stored name is not a record snapshot');
  }
  await api('deletefile', { fileid: fileid });
  return { deleted: true, name: meta.name, fileid: fileid };
}

module.exports = { HOST, listFolder, stat, createFolderIfNotExists,
                   checksum, sha1File, download, upload, withRetry, redact,
                   deleteSnapshot, SNAPSHOT_NAME };
